...
data_dir = '/mbon/data_big/satellite/sst4/anom_27km'
nc_files = list.files(data_dir, '.*\\.nc$', full.names=T)
  
for (nc_path in nc_files){ # nc_path = nc_files[1]
...
```{r extract sst fasterize}
library(fasterize) # devtools::install_github("ecohealthalliance/fasterize")

data_dir  = '/mbon/data_big/satellite/sst4/anom_27km'
grd_files = list.files(data_dir, '.*27km\\.grd$', full.names=T)
grd = grd_files[1]

r = brick(grd) # plot(r[['mean']])
