...
obis_tbl = occurrence(geometry=wkt) %>% 
  as_tibble()
obis_tbl = obis_tbl %>%
  select_(.dots = setdiff(names(obis_tbl), cols_drop)) # names(obis_tbl)
...
eez_sf = jsonstr %>% geojsonio::as.json() %>% geojsonio::geojson_sp() %>% sf::st_as_sf()
mapview::mapview(eez_sf)
plot()
...
# setwd('/mbon/sdg14/technical')
eez_rdata = '/mbon/sdg14/technical/cache/eez.rdata'
if (!dir.exists(dirname(eez_rdata))) dir.create(dirname(eez_rdata))
...
- [ESRI Query (Operation)](https://resources.arcgis.com/en/help/rest/apiref/query.html)
- [ProtectedPlanet ESRI Web Service Query](http://ec2-54-204-216-109.compute-1.amazonaws.com:6080/arcgis/rest/services/wdpa/wdpa/MapServer/1/query)
    - [Spatial Reference, Fields](http://ec2-54-204-216-109.compute-1.amazonaws.com:6080/arcgis/rest/services/wdpa/wdpa/MapServer/1)

```{r wdpa}
bb = st_bbox(cmars_sf)
# bb = sf[1] %>% st_bbox()
# bb = sf %>% filter(NAME=='Malpelo') %>% st_bbox()
