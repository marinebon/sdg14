...
The equation above is problematic because it biases equatorial cells and downweights poleward cells.

$$
E_s = \sum_{c=1}^{C} e_{s,c} * a_c \\
R_{s,c} = \frac{ e_{s,c} * a_c}{ E_s} \\
{RN\ R_c} = \sum_{s=1}^{S} R_{s,c} * e_{s,c}  * w_s
$$

What is 1?
- Extinction risk ($w_s$): 1 = extinct, 0.8 = critically endangered, ...,  0.2 = least concern
- Range normalization ($R_{s,c}$): smallest cell area * . OR use the smallest species range. That becomes 1, per species group, for comparing across groups. OR could change that with a species richness factor.

Alternatively, we describe a range normalization factor per species ($N_s$) that can be applied to all cells ($C$) regardless of area relative to some minimum range size we define as 1.
...
spp_g <- spp_g %>%
  slice(which(spp_g$iucn_category == "NA")) %>%
  mutate(
    genus_species = glue("{Genus} {Species}"),
    iucn_category = map_chr(genus_species, get_iucn_category))

spp_g$iucn_category <- as.character(spp_g$iucn_category)
table(spp_g$iucn_category)
sum(is.na(spp_g$iucn_category))
