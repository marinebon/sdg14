...
roi_geo          = 'cache/cmar.geojson'
occ_csv          = 'cache/cmar_occ.csv'
occ_cklist_csv   = 'cache/cmar_occ_checklist.csv'
occ_txt          = 'cache/cmar_occ_nwkt.txt'
occ_geo          = 'cache/cmar_occ.geojson'
eez_rdata        = 'cache/eez.rdata'
eez_geo          = 'cache/cmar_eez.geojson'
wdpa_geo         = 'cache/cmar_wdpa.geojson'
wdpa_eez_geo     = 'cache/cmar_wdpa_eez.geojson'
occ_eez_wdpa_csv = 'cache/cmar_occ_eez_wdpa.csv'
```
...
Above are the OBIS occurrence points, filtered to the original CMAR polygon (`r occ_nwkt %>% comma()` to `r nrow(occ_sf) %>% comma()` records). Click on the marker cluster to zoom and explode into finer clusters and eventually individual points.

## Fetch EEZs within CMAR
...
If we already knew the country EEZs that overlapped with the CMAR region, we could use the unevaluated code below that Eduardo Klein Sala shared originally from Pieter Provost using the [mregions](https://github.com/ropenscilabs/mregions) package. 
